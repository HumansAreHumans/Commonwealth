      ...
      for (
        ...
        const portalNew = makeButton(18, 'open', (lastClick: ClickHistory) => {
          if (
            ...
            lastClick.lastButton.texture = sheet[14];
            (lastClick.lastButton as any).noSet = true;
            (lastClick.lastButton as any).dest = planet;
            lastClick.currentButton.texture = sheet[14];
            (lastClick.currentButton as any).noSet = true;
            (lastClick.currentButton as any).dest = lastClick.lastButtonOwner;
            return;
          }
        });
        portalNew.position.set(portalIndex * 10, 0);
